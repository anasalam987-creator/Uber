...
def init_db():
    con = get_db()
    con.execute('CREATE TABLE IF NOT EXISTS trips (id TEXT PRIMARY KEY, date TEXT, time TEXT, type TEXT, price REAL, pickup TEXT, dropoff TEXT, zone_pickup TEXT, zone_dropoff TEXT, km REAL, duration INTEGER, rating REAL, boost REAL, notes TEXT, added_by TEXT, created_at TEXT)')
    con.commit()
    con.close()
...
def api_upload():
    ...
    for f in files:
        try:
            ...
            if any(x in mt for x in ['heic','heif']):
                mt = 'image/jpeg'
